...
public class FeatureFiveTest {
    ...
    public void testUserStory14() {
        ...
        clickOnBook(0);
        pressBack();
        openNavFragment(R.id.nav_history,"History");
        onView(withText("ISBN: "+ addedToFavourites.getIsbn())).check(ViewAssertions.matches(isDisplayed()));
        logOut();
        login(username, password);
        openNavFragment(R.id.nav_history,"History");
        onView(withText("ISBN: "+ addedToFavourites.getIsbn())).check(ViewAssertions.matches(isDisplayed()));

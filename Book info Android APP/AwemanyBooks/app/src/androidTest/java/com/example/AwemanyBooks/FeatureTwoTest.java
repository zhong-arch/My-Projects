...
public class FeatureTwoTest {
    ...
    public void userStory4(){
        for(int i = 0; i < bookList.size(); i++){
            ...
            searchByTitleAuthor(currentBook.getTitle(), currentBook.getIsbn());
            searchByTitleAuthor(currentBook.getAuthor(), currentBook.getIsbn());
        }
    }
    @Test
    public void userStory5(){
        for(int i = 0; i < bookList.size(); i++){
            Book currentBook = bookList.get(i);
            searchByISBN(currentBook.getIsbn(), currentBook.getTitle());
        }
    }

    public void searchByTitleAuthor(String toSearch, String toCompare){
        search(toSearch);
        onView(withText("ISBN: "+ toCompare)).check(ViewAssertions.matches(isDisplayed()));
    }

    public void searchByISBN(String toSearch, String toCompare){
        search(toSearch);
        onView(withText(toCompare)).check(ViewAssertions.matches(isDisplayed()));
    }

    public void search(String toSearch){
        onView(withId(R.id.search_input)).perform(ViewActions.click());
        onView(withId(R.id.search_input)).perform(ViewActions.clearText(), ViewActions.typeText(toSearch));

...
public class FeatureOneTest {
    ...
    @Test
    public void testUserStory1And3(){
        onView(withId(R.id.toolbar)).check(ViewAssertions.matches(ViewMatchers.hasDescendant(withText("Home"))));
        onView(withId(R.id.main_recyclerView)).check(ViewAssertions.matches(isDisplayed()));
        for(int i = 0; i < bookList.size(); i++){
            Book currentBook = bookList.get(i);
            checkBooksOnPage(i, currentBook.getTitle(), currentBook.getAuthor(), currentBook.getIsbn());
        }
    }
    ...
    public void testUserStory2(){
        for(int i = 0; i< categories.size(); i++){
            openNavFragment(categoryIds.get(i), categories.get(i));
            testCategoryRecyclerView(categories.get(i));
        }
    }

    public void checkBooksOnPage(int position, String title, String author, String isbn){
        onView(ViewMatchers.withId(R.id.main_recyclerView)).perform(RecyclerViewActions.actionOnItemAtPosition(
                position, ViewActions.scrollTo()));
        ...
        pressBack();
        onView(withText("ISBN: " + isbn)).check(ViewAssertions.matches(isDisplayed()));
    }

    public void testCategoryRecyclerView(String category){
        List<Book> categoryList = new ArrayList();
        Book currentBook;
        for(int i = 0; i< bookList.size(); i++){
            currentBook = bookList.get(i);
            if(currentBook.getCategory() == category) {
                categoryList.add(bookList.get(i));
            }
        }
        for(int j=0; j< categoryList.size(); j++){
            currentBook = categoryList.get(j);
            checkBooksOnPage(j, currentBook.getTitle(), currentBook.getAuthor(), currentBook.getIsbn());
        }
    }
}

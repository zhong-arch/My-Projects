...
public class FeatureEightTest {
    ...
    @Test
    public void testUserStory20and22() {
        clickOnBook(0);
        onView(withId(R.id.comment_recyclerView)).check(ViewAssertions.matches(isDisplayed()));
        onView(withText("local")).check(ViewAssertions.matches(isDisplayed()));
        onView(withText("Awesome!")).check(ViewAssertions.matches(isDisplayed()));
        onView(withText("4.0")).check(ViewAssertions.matches(isDisplayed()));
    }

    @Test
    public void testUserStory19and21(){
        signUp(username, password);
        login(username, password);
        openNavFragment(R.id.nav_home, "Home");
        clickOnBook(1);
        onView(withId(R.id.review)).perform(ViewActions.click());
        SetRating r = new SetRating();
        (onView(withId(R.id.Review_rating))).perform(r);
        onView(withId(R.id.Review_edit)).perform(ViewActions.typeText("What a great book"));
        onView(withId(R.id.Review_submit)).perform(ViewActions.click());
        logOut();
        openNavFragment(R.id.nav_home, "Home");
        clickOnBook(1);
        onView(withText("user")).check(ViewAssertions.matches(isDisplayed()));
        onView(withText("What a great book")).check(ViewAssertions.matches(isDisplayed()));
        onView(withText("3.0")).check(ViewAssertions.matches(isDisplayed()));
    }

...
public class FeatureThreeTest {
    ...
    @Test
    public void testUserStory7and8(){
        for(int i = 0; i<bookList.size(); i++){
            Book currentBook = bookList.get(i);
            recyclerViewClick(i, currentBook.getTitle(), currentBook.getAuthor(), currentBook.getDescription(),
                    currentBook.getIsbn(), currentBook.getAzprice(), currentBook.getAzebprice(),
                    currentBook.getChprice(), currentBook.getChebprice());
        }
    }

    public void recyclerViewClick(int position, String title, String author, String description, String isbn,
                                  String azprice, String azebprice, String chprice, String chebprice){
        ...
        onView(withText(author)).check(ViewAssertions.matches(isDisplayed()));
        onView(withText(description)).check(ViewAssertions.matches(isDisplayed()));
        onView(withText(isbn)).check(ViewAssertions.matches(isDisplayed()));
        onView(withId(R.id.az_price)).check(ViewAssertions.matches((withText("Paper: " +azprice))));
        onView(withId(R.id.azeb_price)).check(ViewAssertions.matches((withText("E-book: "+ azebprice))));
        onView(withId(R.id.ch_price)).check(ViewAssertions.matches((withText("Paper: " + chprice))));
        onView(withId(R.id.cheb_price)).check(ViewAssertions.matches((withText("E-book: "+ chebprice))));
        pressBack();
    }
}
